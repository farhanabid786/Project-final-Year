...
void main() {
  // 1. MASK UV for safety fade
  vec2 maskUv = (gl_FragCoord.xy * 2.0 - uResolution.xy) / uResolution.y;
  
  // 2. EYE UV (Increase 2.2 to make the "canvas" even bigger)
  vec2 uv = maskUv;
  uv *= 2.2; 
  uv /= uScale;
  ...
  // Adjusted pupil squash for better almond shape
  float pupil = 1.0 - length(pupilUv * vec2(9.0, 2.8));
  pupil *= uPupilSize;
  pupil = clamp(pupil, 0.0, 1.0);
  pupil /= 0.35;
  
  // Eye Glow (Almond shape glow)
  ...
  // 3. FINAL EDGE PROTECTION (Fades out everything before hitting corners)
  float edgeFade = smoothstep(1.3, 0.7, length(maskUv)); 
  color *= edgeFade;
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        opacity: isReady ? 1 : 0, transition: 'opacity 0.6s ease-in'
      }} 
    />
  );
}